...
function startExecutionSimulation(incidentId: string) {
  // create a few steps and advance them asynchronously
  if (executions[incidentId]) return; // already running
...
export async function fetchAudit(incidentId?: string) {
  await delay(200);
  if (!incidentId) return auditLogs.slice();
  return auditLogs.filter((a) => a.incidentId === incidentId);
}

export async function fetchExecution(incidentId: string) {
  await delay(150);
  return executions[incidentId] ? executions[incidentId].map((s) => ({ ...s })) : [];
}
...
export async function updateIncident(id: string, updates: Partial<Incident>) {
  ...
  // record audit entry for status or command changes
  if (before) {
    if (updates.status && updates.status !== before.status) {
      addAuditEntry(id, `status:${updates.status}`, `Status changed from ${before.status} to ${updates.status}`);
    }
    if (updates.command && updates.command !== before.command) {
      addAuditEntry(id, `command:modified`, `Proposed command changed`);
    }
  }

  // if status indicates execution, start simulation
  if ((updates as any).status === "Executing") {
    addAuditEntry(id, "execution:started", "Execution started by mock API");
...
export function resetExecution(incidentId: string) {
  ...
  // leave incident status as Pending
  incidents = incidents.map((i) => (i.id === incidentId ? { ...i, status: "Pending" } : i));
  return true;
}
